...
NB_CLASS = 67
PRETRAINED = "extras/abdul_suhani.h5"
...
def evaluate(model, plist, batch_size=16):
    ...
    chunks = pd.read_csv('traintest/race_test.txt',
...
def epsw(model, batch_size=16):
    ...
    chunks = pd.read_csv('traintest/race_test1.txt',
    ...
    for data in chunks:
        ...
        evl = model.evaluate(X_train, y_train, batch_size=4)
        print evl
        with open('outputs/race_preds.txt', 'a') as f:
...
def train(model, batch_size=16, epochs=400, lr=1e-4, nb_epoch=1):
    ...
    for epoch in xrange(169, epochs):
        start = time.time()
        chunks = pd.read_csv('traintest/race_train.txt',
                             names=['person', 'class', 'image'], chunksize=256,
                             sep='\t', engine='python')
        count = 0
        # for data in chunks:
